...
def is_valid_username(username): 
    ...
    # TODO: Tambahkan pola untuk memvalidasi panjang karakter minimal 5 dan maksimal 20. 
    return re.search(pattern, username) 

def is_valid_email(email): 
    pattern = r"^[a-z]+([0-9]{2,})?@[a-z]+\.([a-z]+\.)?(com|co\.id|ac\.id)$"
    # TODO: Buatkan pola RegEx untuk validasi email. 
    # Sudah ada clue berupa pola untuk mencocokkan domain email. 
    return re.search(pattern, email)
 
def is_valid_password(password): 
    pattern = r"^(?=.*[A-Z])(?=.*[a-z])(?=.*[0-9]).{8,}$"
     # TODO: Buatkan pola RegEx untuk validasi password sesuai permintaan soal. 
...
if is_valid_username(username): 
    ...
    if is_valid_email(email): 
        ...
        if is_valid_password(password): 
            print("\nRegistrasi berhasil! ...") 
            # TODO: Tambahkan sapaan "Halo (username)"  
